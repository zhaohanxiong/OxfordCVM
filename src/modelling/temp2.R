...
# start offline plot
png(file.path(path, "temp1.png"), width = 1000, height = 500)

# generate plotting data frame
df_plot = data.frame(y = cut(scores$global_pseudotimes,
                             breaks = seq(min(0, na.rm = TRUE), 
                                          max(1, na.rm = TRUE), 
                                          length = n_intervals)),
                     x = var)
...
p1 = ggplot(aes(y = x, x = y, fill = y), data = df_plot) + 
        geom_boxplot() +
        ggtitle(sprintf("Trend of %s Variable (Per Interval)", 
                                            gsub("_", " ", weights$group[i]))) +
        ylab(sprintf("%s", toTitleCase(weights$name[i]))) + 
        xlab("Hyper Score [0-1] (Regular Intervals)") +
        theme(legend.position = "none",
              axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
              plot.title = element_text(size = 15, face = "bold"))

# aggregate mean of scores by interval
df_median = aggregate(df_plot$x,
                      by = list(group = df_plot$y),
                      "median")
...
p2 = ggplot(df_median, aes(x = group, y = x)) + 
        ...
        ggtitle(sprintf("Trend of %s Variable (Per Interval)",
                                              gsub("_", " ", weights$group[i]))) +
        ylab(sprintf("%s", toTitleCase(weights$name[i]))) + 
        xlab("Hyper Score [0-1] (Median Per Interval)") +
        theme(legend.position = "none",
              axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
              plot.title = element_text(size = 15, face = "bold"))
...
df_plot = aggregate(df_plot$x,
                    by = list(group = df_plot$y, traj = df_plot$traj),
                    "median")
