...
before = 1;
after = 5;
difference = single.empty;
for feature_id = 1:216
...
if strcmp(method, 'cPCA')
    [cPCs, gap_values, alphas, no_dims, contrasted_data, Vmedoid,Dmedoid] = ... 
                cPCA_fs(data(:,before:after),starting_point,final_subjects,max_cPCs,classes_for_colours,alphas_all);
...
% % % dist_matrix_combined = dist_matrix(in_background_target, in_background_target);
% % % for p = 1:size(dist_matrix_combined,1)
% % %     dist_matrix_combined(1:1374,p) = movmedian(dist_matrix_combined(1:1374,p),50);    
% % %     dist_matrix_combined(1375:end,p) = movmedian(dist_matrix_combined(1375:end,p),50);
% % % end
% % % for p = 1:size(dist_matrix_combined,1)
% % %     dist_matrix_combined(p,1:1374) = movmedian(dist_matrix_combined(p,1:1374),50);    
% % %     dist_matrix_combined(p,1375:end) = movmedian(dist_matrix_combined(p,1375:end),50);
% % % end
...
background_scores = median(global_pseudotimes(starting_point));
target_scores = median(global_pseudotimes(final_subjects));

difference(feature_id,1) = target_scores - background_scores

before = before + 5;
after = after + 5;
end

high_difference = find(difference >= 0.07);
features_list = 1:size(data,2);
sets_of_5 = single.empty;
before = 1;
after = 5;
for p = 1:216
    sets_of_5(p,:) = features_list(before:after);
    before = before + 5;
    after = after + 5;
end

selected_features = sets_of_5(high_difference,:);
selected_features = sort(selected_features(:),'ascend');
