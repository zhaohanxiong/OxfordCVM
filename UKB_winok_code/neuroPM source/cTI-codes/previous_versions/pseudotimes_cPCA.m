function [global_ordering,global_pseudotimes,Node_Weights] = pseudotimes_cPCA(data,starting_point,final_subjects,previous_Node_Weights)
% Author: Yasser Iturria medina, 17/05/2018.
% See:
% Iturria-Medina et al., 2019. biorxiv.
% Welch et al., 2016, Genome Biology.

N_nodes = size(data,1);
classes_for_colours                 = 2*ones(N_nodes,1);
classes_for_colours(starting_point) = 1;

if nargin > 3 && ~isempty(previous_Node_Weights) %--- Transforming data, applying a-priori mapping on the data
   mappedX      = zscore(zscore(data)*previous_Node_Weights); 
   Node_Weights = previous_Node_Weights; clear previous_Node_Weights
   contrasted_data = [];   
else %--- Reducing dimensionality using cPCA
    if nargin < 3 || isempty(final_subjects)
        final_subjects = setdiff(1:N_nodes,starting_point)';
        disp('Contrastive PCA of the data ...')
        [cPCs,gap_values,alphas,no_dims,contrasted_data,Vmedoid] = contrastiveKernelPCA(data,starting_point,final_subjects,10,classes_for_colours);
        cPCs = cPCs(1:N_nodes,:,:); contrasted_data = contrasted_data(1:N_nodes,:,:); classes_for_colours = classes_for_colours(1:N_nodes);
    else
        disp('Contrastive PCA of the data ...')
        [cPCs,gap_values,alphas,no_dims,contrasted_data,Vmedoid] = contrastiveKernelPCA(data,starting_point,final_subjects,10,classes_for_colours);
    end
...
datas = dijkstra(MST, starting_point(j)');
% Continuing analysis
max_distance = max(datas.A(~isinf(datas.A)));
global_pseudotimes = datas.A/max_distance; 
[~,global_ordering] = sort(datas.A); 
return;
