function [global_ordering,global_pseudotimes,mappedX,contrasted_data,Node_contributions,Expected_contribution] = pseudotimes_cTI(data,starting_point,classes_for_colours,final_subjects,method,max_cPCs)
%-- INPUTS:
...
% cPCA by definition, PCA and UMAP should be considered only for comparison
% analyses.
...
% Node_contributions: contribution of each node to the final representation
% space.
...
% -------------------------------------------------------------------------
% Author: Yasser Iturria medina, NeuroPM lab, MNI, McGill. 17/05/2018.
% See:
% Iturria-Medina et al., 2019. https://www.biorxiv.org/content/10.1101/548974v1

if nargin < 5 || isempty(method), method = 'cPCA'; end
if nargin < 6 || isempty(max_cPCs), max_cPCs = 10; end

%--- Reducing dimensionality
N_nodes = size(data,1);
if nargin < 4 || isempty(final_subjects), final_subjects = setdiff(1:N_nodes,starting_point)'; end
...
elseif strcmp(method,'PCA')
    ...
    [V,D] = eig(cov(norm_data));
    % sort eigenvalues in descending order
    [D,ind] = sort(diag(D), 'descend');
    % calculating intrinsic dimensionality and reduced srepresentation
    % space
    D = D - min(D);
    Lambdas = D/sum(D);
    ind = find(Lambdas > 0.025);
    mappedX = norm_data * V(:,ind);    
    Node_Weights = V; contrasted_data = [];
    Node_contributions = (100*(Node_Weights.^2)./repmat(sum(Node_Weights.^2,1),size(Node_Weights,1),1))*Lambdas;
    Expected_contribution = sum(100*1/size(Node_Weights,1)*Lambdas);
elseif strcmp(method,'UMAP')
    norm_data = zscore(data);
    mappedX   = run_umap(norm_data); 
