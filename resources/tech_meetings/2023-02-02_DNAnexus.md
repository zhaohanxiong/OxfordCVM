...
6) Data Analysis in R:
    ...
    - To access project data files, we need to use the ```dx``` program provided by UKB RAP, which works in the terminal
    ...
    - Lets say you have performed some analysis, and you want to save your output (stored as the variable ```df_out```) to your own computer
      - ```write.csv(df_out, file = "my_output.csv")``` will write this file to inside your R studio instance
      - ```system("dx upload my_output.csv")``` will then upload this file to the ```/mnt/project``` where it will be accessible in the projects filesystem GUI
      - ```/mnt/project``` is read-only so you have to use ```dx```
      - ```/mnt/project``` is the default base path for any ```dx``` function, so you can pretend that any further paths stems from it
      - e.g. ```system("dx /Bulk/upload my_output.csv")``` will write the data into the ```Bulk``` directory
